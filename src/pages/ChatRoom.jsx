...
const ChatRoom = () => {
  ...
  const [targetMessageForEmoji, setTargetMessageForEmoji] = useState(null);
...
const ChatRoom = () => {
  ...
  const renderMessageContent = (msg) => {
    if (msg.type === 'code') {
      ...
      return renderMessageTextWithTruncation(msg);
    }
  };
  ...
  const toggleMessageTypeFilter = (typeId) => {
    setActiveFilters(prev => {
      ...
      return { ...prev, messageTypes: types };
    });
  };
  ...
  const renderFilterPanel = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-md w-full p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Filter Messages</h3>
            <button 
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {messageTypeOptions.map(option => (
                <button
                  key={option.id}
                  onClick={() => toggleMessageTypeFilter(option.id)}
                  className={`px-3 py-2 rounded-md text-sm ${
                    activeFilters.messageTypes.includes(option.id) 
            ...
            <div className="max-h-48 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md">
              {roomUsers.map(user => (
                <button
                  ...
                  className={`flex items-center p-3 w-full text-left ${
                    activeFilters.fromUsers.includes(user.id) 
                      ? 'bg-primary-50 dark:bg-primary-900' 
                      : 'hover:bg-gray-50 dark:hover:bg-gray-700'
                  }`}
                >
                  ...
                  {activeFilters.fromUsers.includes(user.id) && (
                    <div className="ml-auto text-primary-500 dark:text-primary-400">
                      <FiCheck size={16} />
                    </div>
                  )}
                </button>
          ...
          <div className="mb-4">
            <label className="flex items-center">
              <input
                ...
                className="form-checkbox text-primary-500 dark:text-primary-400"
              />
              <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">With Media</span>
            </label>
          </div>
          ...
          <div className="mb-4">
            <label className="flex items-center">
              <input
                ...
                className="form-checkbox text-primary-500 dark:text-primary-400"
              />
              <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Only Mentions</span>
            </label>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex space-x-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-md bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300"
              />
              <input
                type="date"
  ...
  const renderMessageInput = () => {
    return (
      <div className="flex items-center px-4 py-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
        <div className="flex-1 flex items-center">
          ...
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-3 py-2 rounded-md bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
            
            {showMentions && (
              <div className="absolute top-full left-0 right-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg max-h-48 overflow-y-auto z-10">
                {mentions.map(user => (
                  <button
                    key={user.id}
                    onClick={() => handleMentionSelect(user)}
                    className="flex items-center p-3 w-full text-left hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    <img 
                      src={user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.username)}&background=random`}
                      alt={user.username} 
  ...
  const renderMessage = (msg, index) => {
    ...
    const isFirstMessage = index === 0 || filteredMessages[index - 1].userId !== msg.userId;
    const isLastMessage = index === filteredMessages.length - 1 || filteredMessages[index + 1].userId !== msg.userId;
    
    return (
      <div
        className={`flex items-start ${isOwnMessage ? 'justify-end' : 'justify-start'}`}
        ref={msg.id === lastMessageId ? lastMessageRef : null}
      >
        ...
        <div className="flex flex-col">
          ...
          <div
            className={`bg-gray-100 dark:bg-gray-700 rounded-lg p-3 shadow-sm relative group ${
              isOwnMessage ? 'bg-primary-500 text-white' : 'dark:bg-gray-800'
            }`}
          >
            {renderForwardedBadge(msg)}
            ...
            {msg.type === 'code' && (
              <div className="bg-gray-800 text-white rounded-md p-2 overflow-x-auto">
                <SyntaxHighlighter language={msg.code.language}>
                  {msg.code.content}
                </SyntaxHighlighter>
              </div>
            )}
            ...
            {msg.type === 'poll' && (
              <div className="flex flex-col">
                <div className="font-medium mb-2">{msg.poll.question}</div>
                <div className="space-y-1">
                  {msg.poll.options.map((option, index) => (
                    <button
                      key={index}
                      onClick={() => handlePollVote(msg.id, index)}
                      className={`flex items-center justify-between px-3 py-2 rounded-md ${
                        msg.poll.votedOption === index 
                          ? 'bg-primary-500 text-white' 
                          : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                      }`}
                    >
                      <span>{option}</span>
                      <span className="ml-2">
                        {msg.poll.votes[index]} vote{msg.poll.votes[index] !== 1 && 's'}
// ChatRoom component for real-time messaging
import { useState, useEffect, useRef, useMemo, useCallback } from 'react';
import { useParams, useNavigate, Link } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
...
const ChatRoom = () => {
  ...
  // Function to format message text with mentions, URLs, etc.
  const formatMessageText = (text) => {
    // Simple URL detection and formatting
    const urlRegex = /(https?:\/\/[^\s]+)/g;
    const formatted = text.replace(urlRegex, (url) => {
  ...
  // Render message text with truncation for long messages
  const renderMessageTextWithTruncation = (msg) => {
    const MAX_LENGTH = 300;
    const isExpanded = expandedMessages[msg.id] || false;
    const shouldTruncate = msg.text.length > MAX_LENGTH && !isExpanded;
    
    if (shouldTruncate) {
      return (
        <div>
          {formatMessageText(msg.text.substring(0, MAX_LENGTH))}
